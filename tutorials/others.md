...
dosya update edildiğinde, fiziksel olarak değişiklikler farklı bir yere yazılır.. önce geçici bölgelere (bu bölgeler "journal" olarak adlandırılıyor) değişiklikler yazılır, tüm dosya güncelleme transaction'ı tamamlandığında, ilgili dosyada güncellenecek olan kısım, journal tarafından overwrite edilir.
...
journaling'e alternatiftir. copy-on-write, transaction tamamlandığında, ilgili dosyanın metadata'larını değiştirir ve eski olan kısımlarını geçersiz olarak algılanmasını sağlar. yani transaction sonunda, sadece update edilen bölgeye referenas verilir. journal'deki gibi overwrite söz konusu değildir. bu sebeple update işlemlerinde journaling'e göre daha hızlıdır, fakat fragmentation ihtiyacınıda bi o kadar arttırır. 
...
# Dil modeli (or Language Model or LLM)
...
__Large Language Model (or LLM)__ çok büyük miktarda datalar üzerinde eğitilip, input olarakta alabilen modellerdir.
